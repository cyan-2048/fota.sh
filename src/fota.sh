...
#  echo "$env/check.php?id=$id&cltp=1&fv=$fv&mode=2&type=FIRMWARE&curef=$cu"
#  echo "$data"
...
f_download () {
  local data slv url
  data=$(echo "id=$id&cltp=1&fv=$fv&tv=$tv&mode=2&type=FIRMWARE&curef=$cu&salt=1&vk=1&fw_id=$fwid" | curl -d@- -X POST "$env/download_request.php")
